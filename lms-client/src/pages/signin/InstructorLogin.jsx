  ...
  const mutation = useMutation({
    ...
    onSuccess: (res) => {
      if (res.data.user.role === 'instructor') {
        ...
        toast.error('Please Login With Instructor Account')
      }
    },
    onError: (err) => {
  ...
  return (
    <Container sx={{
      ...
      <Stack sx={{
        ...
        <Typography sx={{ fontSize: '20px', fontWeight: 500, mb: 2, textAlign: 'center' }}>Instructor Login</Typography>
        <TextField
        ...
        {emailNotReceivedSecOpen &&
          <Box sx={{
            ...
            >
              {disableResendBtn ? 'Email Send' : 'Resend Email'}
            </Button>
          </Box>
        }
